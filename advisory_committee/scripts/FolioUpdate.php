...
//print "added id: ".print_r( addIdToArray( array(array('target_id'=>1),array('target_id'=>2)), 2 ), true)."\n"; exit;
...
if( !isset($folioData['fields']['link']) ){ $folioData['fields']['link'] = 'link';  } //link will be the default displayed value
...
function addIdToArray($fvalues, $newId){
	print "Checking array for id: ".$newId."\n";
	$found = false;
	foreach($fvalues as $i=>$targ){
		if($targ && $targ['target_id'] && $targ['target_id']==$newId){
			$found=true; break;
		}
	}
	if($found) return $fvalues;
	array_push($fvalues, array('target_id'=>$newId) );
	return $fvalues;
